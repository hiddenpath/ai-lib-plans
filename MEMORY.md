...
- **Progress (2026-04-03; PT-073 CI slices updated 2026-04-15; WASM verified 2026-04-19)**: Rust workspace `ai-lib-core` + `ai-lib-contact` + `ai-lib-wasm` + facade `ai-lib-rust` + optional `ai-lib-wasmtime-harness`; PT-068/PT-072 **completed** (WASI exports, ~1.3MB release wasm, tests pass). PT-073 **in progress**: WASM compliance **verified** (wasm32-wasip1 target installed, binary builds, wasmtime harness tests pass); full four-runtime + WASM gate checklist: E/P separation and migration docs remain. **on `ailib-official/main`**, Python ships `.github/workflows/pt073-python-e-only.yml` (`COMPLIANCE_SUBSET=e_only` + E/P checks) and TypeScript ships `.github/workflows/pt073-ts-core.yml` (`typecheck` + `npm run test:core` with `AI_PROTOCOL_DIR`) — both check out **ailib-official/ai-protocol**. Core `cargo test -p ai-lib-core --test compliance_from_core` with `COMPLIANCE_DIR` continues to run the full YAML suite where configured; wasmtime harness `cargo test -p ai-lib-wasmtime-harness --test wasm_compliance` after wasm release build **passes**. Governance **GOV-001 v2** + **GOV-002** (merge conflict resolution discipline: no blind whole-file takeover; PR rationale + narrow tests). Checklist: `ai-protocol/docs/WAVE5_V1_GATE_CHECKLIST.md`.
...
**部署脚本**：`ai-lib-plans/tools/deploy_eos.sh` — 一键 Docker 部署自动化（git pull → docker build → export → scp → remote load + restart + health check）

**关键配置**：
- 本地仓库：`/home/alex/eos`
- Docker 镜像：`eos:latest`
- 目标服务器：`43.159.226.236`（CentOS 7），Caddy 反代 `eos.ailib.info`
- 构建代理：`192.168.2.13:8887`
- CN 镜像源已在 Dockerfile 中固化（aliyun apt、USTC cargo/rustup）
- `wasm-pack-bin` 本地二进制 COPY 进容器（GitHub SSL 在 Docker 内不可靠）
- 远程容器：`--env-file /opt/eos/.env.production -v /opt/eos/uploads:/tmp/eos/uploads --restart unless-stopped`
- 脚本支持 `--dry-run`、`--skip-pull/build/upload/restart` 分步跳过
