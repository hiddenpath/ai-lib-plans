...
## 4. `Split-Path -Leaf` 与以 `.desktop` 结尾的目录名（OpenCode）

**症状**：迁移 `%APPDATA%\ai.opencode.desktop` 时脚本报 `AmbiguousParameterSet`，发生在 `Split-Path -LiteralPath $path -Leaf` 一行。

**根因**：Windows PowerShell 5.1 下，路径最后一级形如「`*.desktop`」时，`Split-Path` 的参数绑定会误判（与「扩展名 / 叶子」解析有关）。

**解决**：用 `[System.IO.Path]::GetFileName($path.TrimEnd('\', '/'))` 取目录名；目录存在性用 `[System.IO.Directory]::Exists($path)`，避免对该路径使用 `Test-Path -PathType Container`（在部分组合下也可能触发绑定问题）。
