...
- `ai-lib-contact`：依赖 `ai-lib-core`，重新导出完整 API（向后兼容）
- `ai-lib-rust` 变为 thin facade re-exporting both（过渡期）
...
- `@ailib/core` + `@ailib/contact` 或 `@ailib/ai-lib-ts` 重导出
...
| 向后兼容破坏（用户 import 路径变化）| 社区影响 | 过渡期保留 facade crate re-export |
...
3. **ai-lib-contact** 作为独立包存在，依赖 core，向后兼容现有 API
