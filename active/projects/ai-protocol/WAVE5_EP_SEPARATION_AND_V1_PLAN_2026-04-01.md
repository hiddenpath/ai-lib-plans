...
> 状态：进行中（2026-04-05：PT-067–072 工程主线已闭；四运行时审查并修复 3 项缺陷——Rust/TS `ExecutionUsage` 允许 partial JSON、Go SSE streaming 采集 usage、TS `module-matrix` 标 transport 为 mixed。PT-073 CI 流水线已落地但尚未在默认分支跑稳。**发布列车与 v1.0.0 tag 待你审阅后决定**。）  
> 创建日期：2026-04-01  
