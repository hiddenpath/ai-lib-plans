...
- 新增工作流：
  ...
  - `PT-012`：compliance-first 多模态验证矩阵与跨运行时门禁（待开始）
  - `PT-013`：多模态发布/回滚/开关策略治理（待开始）
  - `PT-014`：v2 生成式多模态 Manifest 扩展 + v1 兼容层（进行中）
  - `PT-015`：ai-protocol-mock P0 provider 多模态行为对齐（待开始）
  - `PT-016`：三运行时 P0 适配与语义一致性收敛（待开始）
  - `PT-017`：spiderswitch 能力路由执行跟进（待开始）
  - `PT-018`：P0 发布列车与回滚就绪治理（待开始）
