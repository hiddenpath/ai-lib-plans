...
> **版本**: v1.1
> **日期**: 2026-06-02（v1.0），2026-06-07（v1.1: 任务型上下文策略 + 二级检索模型）
...
6. **廉价模型做向导**：用一个超小本地模型（~1-3B）代替传统向量索引做文件级检索，理解语义而非仅匹配关键词
...
**检索机制**：Layer 2 的填充可通过一个廉价本地模型（~1-3B 参数，本地运行或远端 API）执行语义检索。该模型不参与主力推理，仅在上下文组装阶段作为"向导"——扫描文件树、定位相关函数/段落、输出相关文件+行号。主力 Provider 只消费检索到的精确片段，而非全量代码。

```
任务输入 → [廉价检索模型] → 命中文件:行号 → [主力 Provider] → 输出
   (几 B)                      (仅相关片段)       (旗舰模型)
```
...
| **Vela 客户端** | 同上 + 可选 E2E 加密同步到云端（Phase 3） + **任务类型识别 + 策略模板加载 + 二级检索模型推理** |
| **Vela 本地侧端** | 廉价模型（1-3B）推理服务，执行代码文件检索，输出结构化检索结果供主力 Provider 消费 |
...
| **Phase 3 \| C**（v1.1 新增） | **二级检索模型集成**（选定模型 + API 封装 + 浏览器端 ONNX 路径） | Phase 3，依赖 Phase 2+\|C |
...
| 形态 | 延迟 | 隐私 | 成本 | 适用场景 |
|:----|:----|:----|:----|:--------|
| 浏览器端 ONNX/WASM | ~10-50ms | ✅ 本地计算 | 免费 | chat、简单 doc |
| 本地 server（Vela 侧） | ~5-10ms | ✅ 本地 | 一次部署 | 编码场景全量 |
| 云端 API（廉价 Provider） | ~200-500ms | ❌ 数据外发 | 按量付费 | 精度要求高、不具备本地算力 |
